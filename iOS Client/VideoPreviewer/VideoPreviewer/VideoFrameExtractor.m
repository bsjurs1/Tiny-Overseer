...
-(void) decodeRawFrame:(VideoFrameH264Raw*)frame callback:(void(^)(BOOL b))callback{
    ...
    @synchronized (self)
    {
		...
		if(got_picture){
			[self.networkManager prepareImage:(uint8_t*) _pFrame ofSize:av_frame_get_pkt_size(_pFrame)];
			//[self.networkManager sendImage:(uint8_t*) _pFrame ofSize: av_frame_get_pkt_size(_pFrame)];
		}
